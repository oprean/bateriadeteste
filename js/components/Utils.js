define([
    ...
    'text!templates/quizzes/result-survy-tpl.html',
    'globals',
    'moment'
    ...
    var Utils = {
        ...
        getTplVarVal: function (varname, model) {
            ...
            switch (varname) {
                case '{quiz.name}':
                    return qtr(tmodel.quiz.name);
                case '{quiz.description}':
                    return qtr(tmodel.quiz.description);

                case '{result.name}':
                    return qtr(tmodel.result.data.result.name);
                case '{result.total}':
                    return tmodel.result.data.result.total;
                case '{result.description}':
                    return qtr(tmodel.result.data.result.description);
                case '{result.groups.total}':
                    return tmodel.result.data.total;
                case '{result.groups}':
                {
                ...
                case '{user.username}':
                    return tmodel.user.username;
                case '{user.firstname}':
                    return tmodel.user.firstname;
                case '{user.lastname}':
                    return tmodel.user.lastname;
                case '{user.email}':
                    return tmodel.user.email;
                default:
                    return varname;
            }
        },
        ...
        renderTemplate: function (model) {
            ...
            if (model.get('quiz').type == Constants.QUIZ_TYPE_SURVEY) {
                template = _.template(resultSurvyTpl);
                console.log(model);
                html = template({
                    quiz:model.get('quiz'),
                    user:model.get('user'),
        ...
        getDefaultPermissions: function (type) {
            switch (type) {
                case Constants.QUIZ_TYPE:
                    return Constants.DEFAULT_PERMISSIONS_FOR_QUIZ;
                case Constants.USER_TYPE:
                    return Constants.DEFAULT_PERMISSIONS_FOR_USER;
                case Constants.GROUP_TYPE:
                    return Constants.DEFAULT_PERMISSIONS_FOR_GROUP;
                case Constants.OPERATION_TYPE:
                    return [];
            }
        },
