...
$app->post('/mail/invite', function () use ($app) {
    $app->response()->header('Content-Type', 'application/json');
    $mailer = new QMail();
    $post = (Object)$app->request()->post();
    $body = file_get_contents(TEMPLATES_DIR.'email'.DS.'invitation.html');
    foreach($post->userIds as $userId) {
        $data = (Object)[
            'user' => $userId, 
            'quiz' => $post->quizId,
        ...
        $invitation = R::findOne(QUIZ_INVITATION_BEAN,'quiz_id=? AND user_id=?',[$post->quizId, $userId]);
        if (!empty($invitation)) {
            $invitation->count++;
            $invitation->modified = date('Y-m-d H:i:s');
        } else {
            $invitation = R::dispense(QUIZ_INVITATION_BEAN);
            $invitation->quiz_id = $post->quizId;
